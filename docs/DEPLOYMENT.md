...
npx prisma migrate deploy  # применить новые миграции (если есть)
npx prisma generate        # если менялась schema.prisma
npm run build
# перезапустить процесс (systemd restart / pm2 restart / etc.)
```

`npx prisma migrate deploy` идемпотентен — безопасно запускать даже если
новых миграций нет.
...
`GET /api/health` — статичный ответ `{ "ok": true, "service": "dental-pro-crm" }`,
без авторизации и без обращения к БД (намеренно — чтобы health check не
зависел от состояния Postgres и не вызывал ложных рестартов при временной
недоступности БД). Подходит для reverse proxy / process manager
(`pm2`/systemd `ExecStartPre`/load balancer healthcheck).

`middleware.ts` явно пропускает `/api/health` без проверки сессии (иначе
неавторизованный запрос получал бы редирект на `/login` вместо JSON).
