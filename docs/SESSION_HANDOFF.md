...
**by AV Systems** · обновлено: 2026-06-14 (после сессии 16: Global Search v1)
...
- `default_appointment_minutes` применён в форме приёма (сессия 13.5,
  ...
  добавляется в список). `reminder_hours_before` и `working_hours` пока
  не читаются потребителями (scheduler) — подключить при доработке.
...
`/admin`, кнопка «Pasiyent məlumat forması» (Tezliklə), **реальная** отправка
...
Global Search v1 закрыт. Варианты по приоритету заказчика:
