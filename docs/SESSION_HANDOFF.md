...
**by AV Systems** · обновлено: 2026-06-28 (после сессии 101: DEMO_PRESENTATION.md Refresh v1)

> Сессии 64-99 не вели хронологию в этом файле (узкие fix/verification-
> сессии — storage/R2, PDF-шрифты, production-верификация публичного demo)
> — их детали зафиксированы в соответствующих профильных доках
> (FREE_DEMO_DEPLOY.md, DOCUMENTS.md и др.), не здесь. §7 ниже обрывается
> на сессии 63; не читать это как «ничего не происходило между 64 и 99».
...
Сценарий показа клинике по ролям — [DEMO_PRESENTATION.md](DEMO_PRESENTATION.md) (сессия 52, обновлено в сессии 101).
...
## 7.41. Сессия 100 — итоги (Demo Reseed Runbook / TZ=UTC Documentation v1)

Docs-only сессия, без изменений кода/схемы/публичной БД. Цель —
задокументировать находку сессии 99, чтобы будущие сессии не переоткрывали
часовой-поясный баг заново.

**Находка сессии 99 (что документируется):** публичный demo на Vercel
держит несколько date-sensitive demo-записей (`prisma/seed.ts`, маркер
«сессия 73» — например «fresh today»-блок Leyla/Profilaktik təmizlik для
`/reports/daily-doctor`), которые освежаются при каждом запуске seed. При
запуске seed **с локальной машины против публичной Neon-базы** (а не как
обычно — на одной машине с самим сервером) обнаружился новый класс
проблемы, отличный от уже известного «урока сессии 71»: Vercel
serverless-функции выполняются в **UTC** (`TZ` не задан), тогда как
локальная машина может быть в другом часовом поясе (в сессии 99 — Asia/Baku,
UTC+4). Seed использует локальные `Date`-методы (`todayAt`/`yesterdayAt`)
— без `TZ=UTC` «сегодняшняя» demo-запись (Leyla) реально попала на день
вперёд относительно того, что сервер считает «сегодня», и отчёт оставался
неполным до второго прогона seed с явным `TZ=UTC`.

**Изменения (эта сессия):**
- **`docs/FREE_DEMO_DEPLOY.md`**: новый §11 «Обновление demo-данных /
  свежесть Gündəlik hesabat» — объяснение проблемы часового пояса, точные
  команды для Git Bash/PowerShell (`TZ=UTC` + `DATABASE_URL` из
  `.env.demo.local`/`DEMO_DATABASE_URL`, никогда не печатать значение),
  жёсткие запреты (никогда `migrate reset`/`db push`/деструктивные cleanup-
  скрипты против публичной базы, никогда против базы реальной клиники),
  чек-лист проверки после освежения. Старый §11 «Устранение проблем» стал
  §12 (одна внутренняя ссылка `§11→§12` исправлена, остальные `§5`/`§9`
  не затронуты — другие секции).
- **`docs/DEPLOYMENT_RUNBOOK.md`** §3: одна строка-указатель на новый §11
  FREE_DEMO_DEPLOY.md.
- **`docs/SESSION_HANDOFF.md`** (этот файл): заголовок обновлён, добавлена
  пометка про пробел сессий 64-99 (не велась хронология здесь — детали в
  профильных доках), этот раздел.

**Не реализовано (намеренно, вне scope):** код/схема/публичная БД не
менялись; backfill хронологии сессий 64-99 в этот файл не делался (узкая
просьба этой сессии — только находка сессии 99 + новый runbook).

**Проверки:** markdown-форматирование вручную просмотрено; секреты в diff
отсутствуют (искал `postgres://`, `DATABASE_URL=`, `DEMO_DATABASE_URL=` с
реальными значениями, R2-ключи — ничего не найдено, во всех примерах
команд используются только имена переменных). `npx tsc --noEmit` не
запускался — изменения чисто в `.md`-файлах, типизированный код не
затронут.

**Один коммит (docs-only):** `docs: document UTC demo reseed runbook`.

## 7.42. Сессия 101 — итоги (DEMO_PRESENTATION.md Refresh v1)

Docs-only сессия, без изменений кода/схемы/публичной БД. Цель — обновить
клиентский demo-сценарий после того, как сессии 92-100 реально подтвердили
на публичном demo то, что раньше существовало только локально: R2-хранилище,
загрузка подписи врача/аватара/лого клиники, генерация PDF («Müalicə
çıxarışı»/«Hesab sənədi») с азербайджанским текстом, и освежение
date-sensitive demo-данных для `/reports/daily-doctor` (с найденным в
сессии 99 нюансом часового пояса — публичный seed нужно запускать с
`TZ=UTC`, см. FREE_DEMO_DEPLOY.md §11).

**Находка при подготовке (architecture note):** старый `docs/DEMO.md`
(сессия 18/52) утверждал, что «загрузка лого клиники и показ в PDF пока
не реализованы» — это уже **частично неверно**: загрузка лого работает
(видна в шапке приложения, сессии 81-83, подтверждено на публичном demo в
сессии 96), но показ лого **именно в сгенерированных PDF** действительно
не реализован (`lib/pdf.ts` рисует только текст реквизитов клиники, без
`doc.image()` для лого — проверено явно). Исправлено точнее в обоих
доках, не как полное «реализовано», а как «загрузка работает, в PDF —
только текст».

**Изменения:**
- **`docs/DEMO_PRESENTATION.md`** — существенно переписан: открывающий
  питч, чек-лист подготовки перед демо (health/db, demo-логины, свежесть
  Gündəlik hesabat), рекомендованный порядок демо из 11 шагов с тезисами
  по каждому (что показываем / акцент для owner / акцент для врача / что
  не обещать на этом экране) — включая новые шаги, которых не было в
  версии сессии 52: Sərfiyyatlar (списание расходников), PDF «Müalicə
  çıxarışı» со встроенной подписью врача, PDF «Hesab sənədi», Gündəlik
  hesabat, бренд клиники/аватар/подпись на `/settings`, объяснение
  multi-tenant изоляции. Добавлены: консолидированный раздел «что не
  обещать» (расширен инфраструктурными оговорками — free-tier demo vs.
  реальная клиника), раздел быстрого восстановления при сбое во время
  демо, финальный sales-style close с шагами после демо. Секции §5/§6/§7
  (recall/feedback-пустота, покрытие demo-данных, что не обещать) сохранили
  номера из версии сессии 52 — внешние ссылки на них не сломаны.
- **`docs/DEMO.md`**: исправлена неточная формулировка про лого клиники
  (см. находку выше), обновлена аннотация ссылки на DEMO_PRESENTATION.md.
- **`docs/RELEASE_CANDIDATE_CHECKLIST.md`** §E: обновлён demo-flow (новые
  шаги), исправлена ссылка `§3-4` → `§4` (секции слиты в новой версии),
  добавлена ссылка на §3 (чек-лист подготовки).
- **`docs/SESSION_HANDOFF.md`** (этот файл): заголовок, аннотация ссылки
  на DEMO_PRESENTATION.md в §2, этот раздел.

**Не реализовано (намеренно, вне scope):** код/схема/публичная БД не
менялись; лого в PDF — не реализовывалось (зафиксировано как известное
ограничение, не фикс).

**Проверки:** markdown-форматирование вручную просмотрено; `git diff --check`
чист (только стандартные LF→CRLF на Windows); секреты в diff отсутствуют
(искал `postgres://`, `DATABASE_URL=`, `DEMO_DATABASE_URL=` с реальными
значениями, R2-ключи, токены — ничего не найдено). `npx tsc --noEmit` не
запускался — изменения только в `.md`-файлах.

**Один коммит (docs-only):** `docs: refresh demo presentation guide`.
