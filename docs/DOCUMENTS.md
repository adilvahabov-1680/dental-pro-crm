...
pdfkit + **DejaVu Sans** — стандартные PDF-шрифты не содержат ə/ş/ğ.
`serverExternalPackages: ["pdfkit"]` в next.config.ts обязателен (webpack
ломает пути к data-файлам pdfkit); дефолтный Helvetica не загружается
(`font:` в конструкторе). Деньги в PDF — «AZN» (знак ₼ есть не во всех шрифтах).

Шрифты — закоммиченные `assets/fonts/DejaVuSans(-Bold).ttf` (сессия 97; источник
и лицензия — `assets/fonts/README.md`), не npm-пакет `dejavu-fonts-ttf` из
node_modules. На Vercel automatic file-tracing не подхватывал .ttf, на который
код ссылается динамической строкой пути внутри pdfkit (не статический
require/import) — файл не попадал в бандл serverless-функции, генерация PDF
падала с `ENOENT`. `next.config.ts` (`outputFileTracingIncludes`) дополнительно
объявляет `assets/fonts/**` явно для всех маршрутов.
