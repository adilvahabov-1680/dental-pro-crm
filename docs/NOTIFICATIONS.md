...
## Дополнение (Session 47): debt reminder queue не трогает эту таблицу

Новая очередь `/finance/debts` (см. [DEBT_REMINDERS.md](DEBT_REMINDERS.md))
переиспользует `prepareInvoiceReminder` (Session 15) — запись остаётся
`channel=whatsapp`, `type=payment_reminder`, как и раньше. `debt_reminder`
(зарезервированный `NotificationType`, см. таблицу выше) по-прежнему
никогда не создаётся как `channel=in_app` — Session 47 не вводит
tenant-level in-app уведомление для долгов, поэтому `TYPE_PERMISSION`
