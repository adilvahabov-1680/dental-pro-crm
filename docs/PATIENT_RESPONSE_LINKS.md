...
local dev/VPS без настройки.

> **На Vercel/serverless — задавать `NEXT_PUBLIC_APP_URL` всегда**, не как
> опциональный edge-case: Vercel — это именно «нестандартный proxy/CDN»
> случай, для которого этот раздел и писался. На реальном demo-деплое
> (сессия 62) пользователь сообщил о неработающей подготовленной ссылке
> `/r/[token]` при отсутствии этой переменной; точную причину той
> конкретной ссылки подтвердить не удалось (исходный токен недоступен для
> повторной проверки), но после явной установки `NEXT_PUBLIC_APP_URL` и
> redeploy свежая сгенерированная ссылка показывает корректный абсолютный
> URL и открывается ожидаемо — см. [FREE_DEMO_DEPLOY.md](FREE_DEMO_DEPLOY.md) §5.
