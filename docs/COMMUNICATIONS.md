...
- `type ∈ {appointment_reminder, payment_reminder, document_message,
  manual_note, reschedule_offer, repeat_visit_reminder, feedback_received}`
  (новые значения enum; `reschedule_offer` — Session 43, см. ниже;
  `repeat_visit_reminder` — существовал в enum с самой первой миграции, но
  реально не использовался до Session 44; `feedback_received` — новое
  значение, Session 45, см. ниже);
...
| `proposeRescheduleOptions` (Session 43) | `appointments.manage` | `getAppointmentForUser` — приём в scope; дополнительно требует `status = reschedule_requested` |
| `prepareRecallMessageAction` (Session 44) | `treatments.manage` | `getRecallTaskForUser` — recall в scope (по пациенту) |
| `prepareFeedbackLinkAction` (Session 45) | `patients.manage` | `getAppointmentForUser`/`getTreatmentItemForUser` — приём/процедура в scope; дополнительно требует `status = completed`/`done` |
...
Все 7 действий — чужой пациент/приём/счёт/документ/recall (другой tenant
или вне ролевого scope, например пациент другого врача для роли
doctor/assistant) → `{ error: "notFound" }`, запись не создаётся
(проверено e2e).
...
**Session 49**: dashboard-reminder проверки больше не зависят от demo-seed
приёмов (раньше — фиксированные 10:00/11:00 «сегодня», падали после этого
времени суток, см. SESSION_HANDOFF.md). Тест создаёт собственные приёмы
относительно текущего момента (+2ч/+3ч) — стабилен в любое время суток без
повторного `npm run db:seed`.
...
## Дополнение (Session 43): reschedule-offer сообщение

Новый шаблон `rescheduleOptionsMessage({ patientName, clinicName,
optionsUrl })` (`lib/communications.ts`) и новое действие
`proposeRescheduleOptions` (`lib/actions/reschedule-options.ts`, отдельный
файл — не `lib/actions/communications.ts`, т.к. логика валидации вариантов
времени принадлежит appointment-домену, а не generic-коммуникациям).
Паттерн идентичен остальным `prepare*`: только текст + wa.me-ссылка +
запись лога (`status="prepared"`) — appointment **не двигается** этим
действием. Полная схема — **[PATIENT_RESCHEDULE_OPTIONS.md](PATIENT_RESCHEDULE_OPTIONS.md)**.

## Дополнение (Session 44): recall / kontrol müayinə сообщение

Новый шаблон `recallMessage({ patientName, clinicName, doctorName? })`
(`lib/communications.ts`) и новое действие `prepareRecallMessageAction`
(`lib/actions/recall-tasks.ts` — отдельный файл, как и `reschedule-options`
в Session 43, т.к. recall — отдельный домен с собственной моделью
`RecallTask`, не просто generic-коммуникация). Используемый
`type = "repeat_visit_reminder"` — не новое значение enum (он существовал
с самой первой миграции, как `PatientResponseLink` до Session 41), просто
...
feedbackUrl, doctorName? })` (`lib/communications.ts`) и новое действие
`prepareFeedbackLinkAction` (`lib/actions/patient-feedback.ts` — отдельный
файл, как и `recall-tasks`/`reschedule-options`, т.к. отзыв — отдельный
домен с собственной моделью `PatientFeedback`). В отличие от остальных
`prepare*` — принимает **либо** `appointmentId`, **либо**
`treatmentItemId` (ровно один из двух), т.к. отзыв может стартовать и от
завершённого приёма, и от завершённой процедуры. Новое значение enum
`type = "feedback_received"` (миграция `20260622000000_add_patient_feedback`,
вместе с новой таблицей `patient_feedbacks`) добавлено и в
`lib/notifications.ts:TYPE_PERMISSION`, иначе staff-уведомление было бы
невидимо в bell. Полная схема — **[PATIENT_FEEDBACK.md](PATIENT_FEEDBACK.md)**.
