...
- `type ∈ {appointment_reminder, payment_reminder, document_message,
  manual_note, reschedule_offer, repeat_visit_reminder}` (новые значения
  enum; `reschedule_offer` — Session 43, см. ниже; `repeat_visit_reminder` —
  существовал в enum с самой первой миграции, но реально не использовался
  до Session 44, см. ниже);
...
Все 6 действий — чужой пациент/приём/счёт/документ/recall (другой tenant
