...
Пароль у всех аккаунтов: **`Demo1234!`** (или значение `SEED_DEMO_PASSWORD`,
если переопределено в `.env`).
...
- **Логотип клиники** — поле есть в профиле, загрузка файла и показ в PDF
  пока не реализованы.
- **Привязка документа к зубу/процедуре** — поле в схеме есть, UI для
  привязки/превью изображений отсутствует.
